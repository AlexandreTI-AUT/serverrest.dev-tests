...
Cypress.Commands.add('dataInput', (email, name, account, password) => {
    cy.window().then((win) => {
...
Cypress.Commands.add('registerAccount', (email, name, password) => {
...
Cypress.Commands.add('transferAccount', (userToTransfer, userRegistered, valueTranfer) => {
    let dataTranfer
...
Cypress.Commands.add('successTransfer', () => {
    cy.get('#modalText').invoke("text").should("equal", "Transferencia realizada com sucesso");
})

Cypress.Commands.add('validateAmount', (userToTransfer, userRegistered, valueTranfer) => {
    cy.getAllLocalStorage().then(res => {
        cy.wrap(res["https://bugbank.netlify.app"][userToTransfer]).then(data => {
...
Cypress.Commands.add('successLogout', () => {
